...
public class GraphView extends AbstractI18NView implements View, ClickDriverActivityQueryListener {
	...
	public void queryDriveActivityClick(ClickQueryEvent event) {
		Driver driverTest;
		try {
			driverTest = driverService.getByCardNumber("E09355108L000000");
		} catch (Exception e) {
		...
		try {
			cardActivityDaily = cardActivityDailyService.getCardActivityDailyByDriver(driverTest, registerDate);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		int size = cardActivityDailyChanges.size();
	}
